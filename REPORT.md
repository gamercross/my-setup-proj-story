...
기존 생산성 도구를 그대로 쓰지 않고 직접 만든 이유는 §1.1에서 다룬 Notion의 한계(대규모 파일 정리 실패)만이 아니다. Notion·Sunsama·Akiflow·Linear 를 목적(§1.0) 기준으로 대조하면 공통된 공백이 드러난다 — 네 서비스 모두 **할일·일정을 정리하는 도구**이지, **경험을 지식으로, 지식을 역량으로 연결하는 도구**는 아니다.

| 특성 | Notion | Sunsama | Akiflow | Linear | 본 프로젝트 |
|---|---|---|---|---|---|
| 데이터 저장 | 클라우드(자사 서버) | 클라우드 | 클라우드 | 클라우드 | 로컬 SQLite, 오프라인 동작 |
| 대규모 자료 정리 | 페이지 계층 구조 — 자료가 늘면 구조가 무너짐(§1.1) | 할일 중심, 자료 정리 기능 약함 | 할일 중심, 자료 정리 기능 약함 | 이슈 추적 중심 | 프로젝트 구조를 앱 안에서 트리로 열람(§5, P9) |
| 지식→역량 연결 | 없음 — 정리 도구일 뿐 | 없음 | 없음 | 없음 | OKR + 기대정렬 체크인으로 지식 축적을 역량 방향과 연결 |
| 자기 점검 방법론 내장 | 없음, 사용자가 직접 구성 | 없음 | 없음 | 없음 | 기대정렬 7질문이 1급 기능(ADR-0035) |
| AI 자동화 | 서드파티 애드온 | 없음 | 없음 | 없음 | 에이전트가 Daily Brief·자동 분류 수행, 사용자 태그는 침범하지 않음(§2.3) |
| 비용 | 유료 플랜 필요 | 월 구독 | 월 구독 | 팀 단위 유료 | 인프라 비용 0원 목표(C-4) |

이 표의 함의는 "더 많은 기능"이 아니라 "다른 대상"이다 — 기존 도구는 **일**을 관리하고, 본 프로젝트는 **일에서 나온 지식**을 관리한다.
...
리스크 레지스터(`RISKS.md`)에서 등급이 가장 높은 항목은 다음과 같다.

| ID | 리스크 | 등급 | 완화책 |
|---|---|---|---|
| R-1 | 1인 개발 + 시험 기간 겹침으로 Phase D 이후 시간 부족 | 🔴 높음 | P0(문서) 우선 완성 후 착수, 일정이 빠듯한 구간을 앞당겨 배치, 범위가 큰 항목은 착수 전에 자름 |
| R-5 | Google OAuth 앱 검토·승인 지연(Gmail/Calendar 스코프) | 🔴 높음 | "테스트 사용자" 모드로 본인 계정만 사용해 검토 절차 자체를 우회 |
| R-13 | 시크릿·토큰이 저장소에 커밋됨(공개 저장소이므로 치명적) | 🟠 중간 | `.gitignore`, 커밋 전 diff 육안 확인. §4.2 S5 에서 지적한 pre-commit 스캔 부재는 이 리스크의 남은 구멍이다 |

이 외에도 `better-sqlite3` 네이티브 빌드 실패(R-4, 3-OS CI 매트릭스로 완화), Claude API 스펙 변경(R-8, 모델 상수 1곳 관리로 완화) 등 기술 리스크를 등급별로 관리한다. 전체 목록과 완화 상태는 `RISKS.md`에 있다.
...
본 프로젝트는 3개 강의에 같은 산출물을 제출하되, 강의마다 보는 층이 다르다.

| 강의 | 평가 배점 | 본 프로젝트의 대응 |
|---|---|---|
| A. AI 컴퓨터 운영체제 실습 | 출석 20·정기 30·수시 20·실험실습성과 30 | 실습성과 = `verify.sh` 통과 로그와 주차별 실습, 수시 = 중간 발표(Ubuntu 웹서비스 데모) |
| B. AI시대소프트웨어공학 | 수시 30·정기 30·출석 20·실습 20 | 수시 = `/feature` 파이프라인 실행 녹화 + 프롬프트 파일, 실습 = 주차별 산출물 |
| C. AITool기반소프트웨어공학 | 수시 30(Term Project)·정기 30(필기)·출석 20·주별리포트 20 | 수시 = 본 보고서를 포함한 설계 산출물(C4·ADR·TRACEABILITY) |

세 강의 모두 정기평가(필기·단답형)는 교재 기반이라 프로젝트와 직접 연결되지 않으며, 이는 사전에 확인된 범위 밖 항목이다.

### 8.2 프로젝트 자체의 성공 지표

강의 배점과 별개로, 프로젝트 자체가 "잘 되고 있는가"를 판단하는 기준은 다음과 같다.

- **검증 게이트 통과율** — `verify.sh`·`check-docs.sh`·백엔드/프런트엔드 테스트가 매 기능마다 100% 통과 상태를 유지하는가 (현재 유지 중, §3)
- **결정의 추적 가능성** — 모든 아키텍처 선택이 ADR로 근거를 남겼는가 (현재 35건)
- **약점의 정직한 기록률** — 발견된 구조적 한계가 은폐되지 않고 원인·영향·보완 방향과 함께 문서화됐는가 (§4.2)
- **목적 정합성** — 새 기능이 §1.0의 목적(지식의 시각화·역량 방향 가시화)에 답하는지, 추가되기 전에 확인하는가

### 8.3 향후 활용 방안

이 프로젝트의 산출물은 강의 제출로 끝나지 않는다. 코드·문서·쇼케이스 사이트는 **포트폴리오 자료**로 재사용하며, ADR·요구사항·C4 문서 체계는 **후속 프로젝트에서 그대로 가져다 쓸 수 있는 방법론 템플릿**으로 의도되었다 — RAW_STORIES.md #8에서 밝힌 "지금까지의 공부에 대한 지식들을 모아 성장할 발판을 만든다"는 도전이 이 활용 방안으로 이어진다.

### 8.4 개인정보 처리

본 보고서와 관련 저장소(`my-setup-proj-story`)에는 작성자의 실명과 개인적 경험(고교 시절 활동 등)이 원문 그대로 포함되어 있다. 이는 실수로 노출된 것이 아니라, 강의 제출·발표라는 목적에 맞춰 **작성자가 명시적으로 공개를 확인한 상태**다(2026-09-14). 이메일 등 추가 식별 정보는 마스킹해 게시했다.
...
7. Google re:Work, *"Set goals with OKRs"* — https://rework.withgoogle.com/en/guides/set-goals-with-okrs (§2.4 OKR 채점 방식의 근거)
8. Doerr, J. (2018). *Measure What Matters.* Portfolio/Penguin. (OKR committed/aspirational 구분의 대중화 저작)
9. Nygard, M. (2011). *"Documenting Architecture Decisions."* — https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions (§2, ADR 관행의 원출처)
10. Brown, S. *The C4 model for visualising software architecture* — https://c4model.com (§3.6, 설계 문서 방법론)
11. arc42 — https://arc42.org (문서 구조 템플릿, §3.6과 함께 채택)
