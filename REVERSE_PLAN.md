...
### 1-2. 고객과 니즈 (2026-09-15)

이 프로젝트를 "보는" 사람은 한 명이 아니다 — 쓰는 사람과 평가하는 사람의 니즈가 다르고,
두 니즈가 같은 산출물 안에서 동시에 풀려야 한다.

| 고객 | 상황 | 니즈 | 이 프로젝트의 답 |
|---|---|---|---|
| **1. 실사용자 (나 자신)** | 고교 PC방 알바·영상 편집·3년 방송부를 거치며 경험은 쌓였지만 기록·정리가 계속 무너짐. Notion도 파일이 늘자 구조가 무너짐 | 혼자서 생각→행동→기록→정리를 전부 감당할 도구, 완료율이 아니라 "무엇을 배웠는지"가 쌓여 보이는 자리 | 기대정렬 체크인 + 프로젝트 내장 OKR + 지식 가시화가 목적인 위젯 셸(§1-0) |
| **2. 강의 평가자 (3개 강의 교수·심사)** | 발표·서면으로 짧은 시간 안에 완성도와 개발 과정의 깊이를 판단해야 함 | 결정 과정의 근거, 검증 가능한 완성도, 실습 주제와의 정합 | ADR 35건·검증 게이트(§3-4)·이 문서 자체 |
| **3. 향후 열람자 (포트폴리오·채용 담당자)** | 완성된 결과물만으로는 실제 역량과 의사결정 과정을 알기 어려움 | 코드 + 결정 근거 + 실패·약점까지 정직하게 드러난 기록 | §4-5 약점 공개, §5-2 재검토 노트 |

세 니즈는 부딪히지 않는다 — 모두 같은 한 문장으로 풀린다: **"경험이 어떻게 지식이
되고, 그 지식이 어느 방향으로 역량을 키우는지 정직하게 보여라."** 사용자는 그걸 매일
쓰는 도구로, 평가자는 그걸 증명하는 산출물로, 향후 열람자는 그걸 증언하는 기록으로
읽는다 — 그래서 "발표용" 축약본을 따로 만들지 않고, 같은 문서·같은 코드를 그대로
공개한다(§6-3 개인정보 처리 참고).
...
**니즈 → 기능 매핑 (2026-09-15).** 위 비교표에서 드러난 공백을, 이 프로젝트의 구체적
기능으로 어떻게 메우는지 짝짓는다.

| 비교에서 드러난 니즈 | 기존 서비스의 공백 | 이 프로젝트가 해결하는 방식 |
|---|---|---|
| "경험을 지식으로 남기고 싶다" | Notion 등은 메모 저장만 하지 구조화된 자기 점검이 없음 | 기대정렬 7질문 체크인 — 뭘·왜·언제까지·목표·전략·구체적으로·상태를 매번 구조화해 기록 |
| "이 지식이 역량으로 이어지는지 보고 싶다" | Linear·Akiflow는 작업 상태만 보여주고 성장 방향은 안 보여줌 | 프로젝트 내장 OKR — 기대정렬 답변이 그 프로젝트 Objective·KR의 기초 데이터가 됨(§2-3) |
| "자료가 늘어도 정리가 안 무너졌으면 한다" | Notion은 페이지가 늘수록 계층이 무너짐(§1-1) | 로컬 SQLite 고정 스키마 + 앱 안 파일 트리·문서 뷰(§5 P9) |
| "작업물이 실제 어디 있는지 확인하고 싶다" | 넷 다 코드 저장소와 분리된 별도 앱 | 프로젝트 카드 안에 GitHub 최근 커밋 내장 — 지식(OKR)과 실물(커밋)이 한 화면 |
| "돈 안 들이고 계속 쓰고 싶다" | 넷 다 유료 플랜 필요 | 로컬 우선 + 무료 계층만(C-4), 인프라 비용 0원 목표 |
...
§4-5의 9개 항목을 텍스트로 훑으면 개별 사실처럼 보이지만, 실제로는 대부분 같은
두 원인에서 갈라져 나온다.
...
flowchart TB
  ...
  SEC --> S1["S1 로컬 네트워크 노출<br/>같은 Wi-Fi 기기가 CRUD 가능"]
  SEC --> S2["S2 CORS null 전 빌드 허용"]
  ...
  S1 -.해소 전제.-> FIX1["다중 사용자 전환(Phase E) 또는<br/>패키징 배포 확정 시점"]
  QUAL -.해소 전제.-> FIX2["1인 개발 속도 제약이 풀리는 시점<br/>(팀 합류 또는 마감 이후)"]
```

즉 9개 약점은 무작위 버그 목록이 아니라, **"지금은 안 해도 되는 것으로 미룬 것"**
이라는 하나의 판단이 여러 갈래로 나타난 결과다 — 다중 사용자 전환이나 마감 이후
시점이 오면 왼쪽 가지(보안)부터 되짚어야 한다.
...
### 5-3. 정합성 감사 — 기능이 목적(§1-0)에 맞는가 (2026-09-14)

§1-0 재정의 이후, 지금까지 만든 기능을 하나씩 다시 세운 목적("완료율이 아니라 지식.
지식이 쌓여 역량이 느는 방향을 한눈에") 기준으로 대조했다.

| 기능 | 정합도 | 이유 |
|---|---|---|
| 진행 현황·파일 탐색(P9) | **강함** | 프로젝트가 쌓은 지식(ADR·결정)을 그대로 노출 — 목적의 대표 사례 |
| 기대정렬 체크인(P10) | **강함** | 7질문 자체가 "경험→지식→역량 방향" 서사 |
| OKR + 구글 등급 | 보통 | committed/aspirational 은 방향성을 담지만, 숫자 달성률 중심이라 여전히 "완료율"처럼 보이기 쉽다 |
| 할일 자동 분류(태그) | 보통 | 가벼운 지식 분류이지만 "무엇을 배웠는지"까지는 안 간다 |
...
| **지식 축적 추세·역량 지도** | **공백** | 개별 위젯은 있지만 "역량이 어느 방향으로 크는지"를 종합해 보여주는 뷰가 없다 |

**판단.** 강함 2·보통 2·인프라 3(정상)·약함 2·공백 2. 지금 있는 기능을 급하게 고칠
필요는 없다 — "약함" 두 항목은 존재 이유가 다른 목적(Q1 통합·Q5 지식 가시화)에
있었고 그 자체로는 유효하다. 다만 **공백 2개**(레퍼런스 요약 절차, 지식 축적
추세·역량 지도)는 §1-0 목적에 아직 아무도 답하지 않은 자리이며, §6-2 남은 일에
다음 세션 후보로 옮긴다.
...
- **다음 세션 기능 후보(§5-3 정합성 감사)** — 레퍼런스 자료 요약 절차 추적(RAW_STORIES #7) ·
  지식 축적 추세·역량 지도 뷰(기대정렬 체크인·OKR 데이터를 시계열로 종합). 둘 다
  §1-0 목적에 아직 아무 기능도 답하지 않은 공백이다.
...
### 6-4. 수치로 보는 성과, 그리고 끝나면 남는 것 (2026-09-15)

진척률 자체를 목적으로 삼지는 않지만(§1-0), 발표·평가를 위해 "어느 정도 해냈는지"는
숫자로 확인할 수 있어야 한다.

| 지표 | 값 |
|---|---|
| 채택된 ADR | 35건 (결정 1개 = 파일 1개) |
| 자동 테스트 | 351건 — backend 157 · frontend 115 · agent 79 |
| `verify.sh` | 49 / 0 / 0 (통과/경고/실패) |
| `check-docs.sh` | 11 / 0 / 0 |
| 실제 위젯 종류 | 10종 — 전부 전용 화면 |
| 완료된 빌드 페이즈 | P0–P9 (10/10) + P10(OKR 등급·기대정렬 체크인) 확장 |
| 강의 공통 산출물 | 3개 강의 동시 제출 |
| 인프라 비용 | 0원 — 로컬 우선 + 무료 계층만 |

**이 프로젝트를 마치면 얻는 것.**

1. **실사용 가능한 개인 도구** — 다음 학기·다음 프로젝트에서도 계속 쓸 수 있는, 내 경험을
   지식으로 쌓는 실제 앱. "만들고 끝"이 아니라 졸업 이후로 이어지는 습관의 도구.
2. **1인으로 설계~빌드~운영을 끝낸 경험** — 요구사항 분해부터 ADR 결정, 에이전트 파이프라인
   설계, 검증 게이트 구축까지 팀 없이 혼자 전체 소프트웨어 생애주기를 완주한 경험치.
3. **재사용 가능한 방법론 템플릿** — ADR·C4/arc42 문서 체계, 4역할 에이전트 파이프라인,
   기대정렬 7질문 — 다음 프로젝트에 그대로 옮겨 쓸 수 있는 틀.
4. **포트폴리오로 바로 쓸 수 있는 증거** — 코드·문서·쇼케이스가 한 세트로 공개돼 있어,
   "무엇을 만들었나"뿐 아니라 "왜 그렇게 결정했나"까지 채용·심사 담당자에게 보여줄 수 있다.
5. **정직하게 약점을 진단하는 습관** — §4-5·5-2처럼 "잘한 것"만이 아니라 "아직 감당
   못 하는 지점"을 스스로 찾아 기록하는 연습. 이 습관 자체가 기대정렬이 노린 역량이다.
